...
function AppContent() {
  ...
  // Sync dark mode class to HTML element for Tailwind CSS variables
  useEffect(() => {
    if (isDark) {
  ...
  const handleApproveCommands = useCallback(
    (ids: string[]) => {
      ids.forEach((id) => {
      ...
      addToast(
        'success',
        `${ids.length} command${ids.length > 1 ? 's' : ''} approved`,
        'Commands will be applied to the device.',
      );
      triggerRefresh();
    },
  ...
  const handleRejectCommands = useCallback(
    (ids: string[]) => {
      ids.forEach((id) => {
        const cmd = getQueue().find((c) => c.id === id);
        rejectCommand(id);
      ...
      addToast(
        'warning',
        `${ids.length} command${ids.length > 1 ? 's' : ''} rejected`,
        'Commands have been removed from the queue.',
      );
      triggerRefresh();
    },
    [triggerRefresh, addToast],
  ...
  return (
    <div className="flex w-full h-full overflow-hidden">
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full">
        ...
        <main className="flex-1 overflow-hidden relative">
          <ErrorBoundary isDark={isDark} resetKey={activeNav}>
            ...
            {activeNav === 'logs' && (
              <div className="h-full overflow-hidden">
                <Logs isDark={isDark} mode={mode} onAuditLog={triggerRefresh} activeDeviceId={activeDeviceId} />
              </div>
            )}
      ...
      {/* Panels */}
      <AuditLogPanel isDark={isDark} isOpen={auditOpen} onClose={() => setAuditOpen(false)} refreshKey={refreshKey} />
      <CommandQueuePanel
        isDark={isDark}
        isOpen={queueOpen}
