...
import {
  Server, Cpu, HardDrive, Clock, Thermometer, Wifi, WifiOff,
  AlertTriangle, CheckCircle, XCircle, Activity, ArrowRight,
  RefreshCw, Zap, Shield, Globe,
} from "lucide-react";
...
export function FleetDashboard({ isDark, mode, onDeviceSelect, setActiveNav }: FleetDashboardProps) {
  ...
  DEVICE_PROFILES.forEach((d) => {
    ...
    if (d.status === "warning") {
      alerts.push({ severity: "warning", device: d.name, message: `${d.model} has degraded performance — check ${d.ip}` });
    }
    const health = healthData[d.id];
    if (health && health.cpu > 70) {
      alerts.push({ severity: "warning", device: d.name, message: `CPU usage at ${health.cpu}% — consider investigating` });
    }
    if (health && health.memory > 80) {
      alerts.push({ severity: "warning", device: d.name, message: `Memory usage at ${health.memory}% — high pressure` });
    }
  });
  ...
  return (
    <div style={{ padding: 24, fontFamily: ui, color: t.text }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h2 style={{ color: t.text, margin: 0, fontSize: 16, fontWeight: 600 }}>Fleet Overview</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
        <AggregateCard isDark={isDark} icon={<Server size={16} color={t.accent} />} label="Total Devices" value={String(DEVICE_PROFILES.length)} sub="managed" />
        <AggregateCard isDark={isDark} icon={<CheckCircle size={16} color={t.green} />} label="Online" value={String(onlineDevices.length)} sub="healthy" color={t.green} />
        <AggregateCard isDark={isDark} icon={<AlertTriangle size={16} color={t.amber} />} label="Warning" value={String(warningDevices.length)} sub="degraded" color={t.amber} />
        <AggregateCard isDark={isDark} icon={<XCircle size={16} color={t.red} />} label="Offline" value={String(offlineDevices.length)} sub="unreachable" color={t.red} />
      </div>
            ...
            {alerts.map((alert, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "8px 16px", borderBottom: i < alerts.length - 1 ? `1px solid ${t.border}` : "none",
                }}
              >
                {alert.severity === "error" ? <XCircle size={13} color={t.red} /> :
                 alert.severity === "warning" ? <AlertTriangle size={13} color={t.amber} /> :
                 <CheckCircle size={13} color={t.accent} />}
                <span style={{ fontSize: 11, fontWeight: 600, color: t.text, minWidth: 100 }}>{alert.device}</span>
                <span style={{ fontSize: 11, color: t.textMuted, flex: 1 }}>{alert.message}</span>
              </div>
            ))}
          </div>
        </div>
...
function AggregateCard({
  isDark, icon, label, value, sub, color,
}: {
  isDark: boolean;
...
function DeviceCard({
  device, health, isDark, loading, onClick,
}: {
  ...
  return (
    <div
      ...
      style={{
        background: t.surface, border: `1px solid ${t.border}`, borderRadius: 10,
        padding: 16, boxShadow: t.shadow, cursor: "pointer",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            ...
            {device.status === "offline" ? <WifiOff size={15} color={statusColor} /> : <Wifi size={15} color={statusColor} />}
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{device.name}</div>
            <div style={{ fontSize: 10, color: t.textMuted }}>{device.model}</div>
          </div>
        </div>
        <div style={{
          display: "flex", alignItems: "center", gap: 4, padding: "3px 8px",
          background: statusBg, borderRadius: 99, border: `1px solid ${statusColor}33`,
        }}>
          <div style={{
            width: 5, height: 5, borderRadius: "50%", background: statusColor,
            boxShadow: device.status === "online" ? `0 0 5px ${statusColor}` : "none",
          }} />
          <span style={{ fontSize: 10, fontWeight: 500, color: device.status === "online" ? t.greenText : device.status === "warning" ? t.amberText : t.redText, textTransform: "capitalize" }}>
            {device.status}
          </span>
        </div>
      </div>
      ...
      {device.status !== "offline" && health && (
        <div style={{
          ...
          <MiniMetric label="CPU" value={`${health.cpu}%`} color={health.cpu > 70 ? t.red : health.cpu > 50 ? t.amber : t.accent} isDark={isDark} />
          <MiniMetric label="RAM" value={`${health.memory}%`} color={health.memory > 80 ? t.red : health.memory > 60 ? t.amber : t.green} isDark={isDark} />
      ...
      <div style={{
        ...
        paddingTop: 8, borderTop: `1px solid ${t.border}`, marginTop: 8,
      }}>
        <span style={{ fontSize: 10, color: t.accent, display: "flex", alignItems: "center", gap: 4 }}>
...
function InfoRow({ label, value, mono, t }: { label: string; value: string; mono?: string; t: ReturnType<typeof getTheme> }) {
  return (
    <div>
      <div style={{ fontSize: 9, color: t.textSubtle, textTransform: "uppercase", letterSpacing: "0.05em" }}>{label}</div>
      <div style={{ fontSize: 11, color: t.text, fontFamily: mono, marginTop: 1 }}>{value}</div>
    </div>
  );
}
