...
export function Config({ isDark, mode, safety, onQueueChange, onOpenQueue, activeDeviceId }: ConfigProps) {
  ...
  const [selectedSection, setSelectedSection] = useState<ConfigSection>(
    configSections[0]?.children?.[0] ?? configSections[0],
  );
  ...
  const pendingChanges = [
    ...
    {
      type: 'added' as const,
      line: 'add action=drop chain=input comment="Drop all other input" log=yes log-prefix="INPUT-DROP"',
      lineNum: 56,
    },
    {
      type: 'added' as const,
      line: 'add address-pool=Guest-Pool disabled=no interface=wlan2 name=dhcp-guest lease-time=2h',
      lineNum: 78,
    },
  ];
  ...
  const enhancedDiffLines: ConfigDiffLine[] = [
    {
      ...
      oldLineNum: 53,
      newLineNum: 53,
      line: 'add action=accept chain=input comment="Allow SSH from LAN" dst-port=22 protocol=tcp src-address=192.168.1.0/24',
    },
    {
      type: 'context',
      oldLineNum: 54,
      newLineNum: 54,
      line: 'add action=accept chain=input comment="Allow Winbox from LAN" dst-port=8291 protocol=tcp src-address=192.168.1.0/24',
    },
    ...
    },
    {
      ...
      oldLineNum: 57,
      newLineNum: 57,
      line: 'add action=accept chain=forward comment="Allow established/related" connection-state=established,related',
    },
    ...
    },
    {
      ...
      oldLineNum: 79,
      newLineNum: 80,
      line: 'add address-pool=LAN-Pool disabled=no interface=bridge1 name=dhcp-lan lease-time=1d',
    },
  ];
    ...
    return content.split('\n').map((line, i) => {
      ...
      return (
        <div key={i} className="flex">
          ...
          {trimmed.startsWith('/') ? (
            <span style={{ fontWeight: 600 }} className="text-t-accent">
              {line}
            </span>
          ) : trimmed.startsWith('add ') || trimmed.startsWith('set ') ? (
            <span>
              <span style={{ color: isDark ? '#7DD3FC' : '#0369A1' }}>{line.match(/^\s*(add|set)/)?.[1]}</span>
              <span className="text-t-text">
                {line
                  ...
                  .map((part, j) =>
                    part.match(/^\w+=$/) ? (
                      <span key={j} style={{ color: isDark ? '#86EFAC' : '#16A34A' }}>
                        {part}
                      </span>
                    ) : part.startsWith('"') || part.endsWith('"') ? (
                      <span key={j} style={{ color: isDark ? '#FCA5A5' : '#DC2626' }}>
                        {part}
                      </span>
                    ) : (
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', fontFamily: ui }}
    ...
    >
      ...
      {mode === 'pro' && (
        ...
        >
          ...
          <span style={{ fontSize: 11 }} className="text-t-text">
            <strong>Auto-revert enabled</strong> — Configuration will automatically roll back if the device becomes
            unreachable after applying changes.
          </span>
        </div>
      )}
      ...
      >
        ...
        <div
          style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 9px', borderRadius: 7 }}
          className="bg-t-amber-bg border border-t-amber"
        >
          <AlertTriangle size={12} color={t.amber} />
          <span style={{ fontSize: 11, fontWeight: 700 }} className="text-t-amber-text">
            High Risk
          </span>
          <span style={{ fontSize: 11 }} className="text-t-text-muted">
            Firewall, DHCP, wireless changes detected
          </span>
        </div>
          ...
          <SafeActionButton
            ...
            onClick={() => {
              ...
              logAuditEntry(
                ...
                'pending',
                `${lines.length} commands queued from ${selectedSection.path}`,
                'Medium',
              );
              onQueueChange?.();
            }}
      ...
      {writeLocked && (
        <div
          ...
          className="bg-t-amber-bg border-b border-t-amber"
        >
          <AlertTriangle size={13} color={t.amber} />
          <span style={{ fontSize: 11, fontWeight: 800 }} className="text-t-amber-text">
            Write actions locked
          </span>
          <span style={{ fontSize: 11 }} className="text-t-text-muted">
            Session is offline or read-only. Validate and preview remain available; apply requires full online access.
          </span>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {viewMode === 'structured' && (
          <div
            ...
            className="border-r border-t-border bg-t-surface"
          >
            <div style={{ padding: '14px 14px 10px' }} className="border-b border-t-border">
              <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }} className="text-t-text">
                Configuration
              </div>
            ...
            <>
              ...
              >
                <div className="flex items-center gap-1.5">
                  ...
                  <span style={{ fontSize: 12, fontFamily: mono }} className="text-t-text-muted">
                    {selectedSection.path}
                  </span>
                </div>
              ...
              <div className="flex-1 overflow-auto py-4">
                {selectedSection.content ? (
                  ...
                  <div style={{ padding: 24, fontSize: 12, fontFamily: mono }} className="text-t-text-muted">
        ...
        {showDiff && (
          ...
          >
            ...
            <div style={{ padding: 12 }} className="border-b border-t-border">
              <div className="grid grid-cols-[1fr_1fr] gap-2 mb-2.5">
                <RiskMetric label="Risk Level" value="High" tone="warning" t={t} />
                <RiskMetric label="Last Backup" value={lastBackup} tone="safe" t={t} />
              </div>
              <div style={{ padding: 10, borderRadius: 8 }} className="bg-t-surface2 border border-t-border">
                <div
                  style={{
                    fontSize: 10,
                ...
                <div className="flex gap-1.5 flex-wrap mb-[9px]">
                  {affectedAreas.map((area) => (
                    <span
                      key={area}
                      style={{ padding: '3px 7px', borderRadius: 999, fontSize: 10, fontWeight: 700 }}
                ...
                <div style={{ fontSize: 11, lineHeight: 1.5 }} className="text-t-text-muted">
                  Potential impact: remote access may be blocked, DHCP leases may change, or wireless users may
                  reconnect. Apply only after validation and backup.
                </div>
              </div>
            </div>
            <div className="flex-1 overflow-auto p-3">
              <div className="grid grid-cols-[repeat(3,1fr)] gap-1.5 mb-2.5">
              ...
              >
                <div
                  ...
                  className="bg-t-surface2 border-b border-t-border text-t-text-subtle"
                >
                  <div className="pl-2">Old</div>
                  <div>New</div>
                  <div>±</div>
                  <div>Command</div>
                  <div>Area</div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        style={{
                          padding: '7px 0',
                          color: isAdded ? t.greenText : isRemoved ? t.red : t.textSubtle,
                          fontFamily: mono,
                          fontWeight: 800,
                          fontSize: 12,
                          textAlign: 'center',
                      ...
                      <div
                        style={{
                          padding: '7px 8px',
                          color: isAdded ? t.greenText : isRemoved ? t.red : t.text,
                          fontFamily: mono,
                          fontSize: 10,
                          lineHeight: 1.55,
                      ...
                      <div className="px-2 py-[7px]">
                        <span
                          style={{ padding: '2px 6px', borderRadius: 999, fontSize: 9, fontWeight: 700 }}
                          className="bg-t-surface border border-t-border text-t-text-muted"
                        >
                          {area}
                        </span>
                      </div>
...
function DiffMetric({
  ...
  return (
    <div style={{ padding: '7px 9px', borderRadius: 8, background: bg }} className="border border-t-border">
      <div
        style={{ fontSize: 9, marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.05em' }}
        className="text-t-text-subtle"
      >
        {label}
      </div>
...
function DiffLineCell({ value, t, mono }: { value: string; t: ReturnType<typeof getTheme>; mono: string }) {
  return (
    <div
      style={{ padding: '7px 6px', fontFamily: mono, fontSize: 10, textAlign: 'right', userSelect: 'none' }}
      className="text-t-text-subtle"
    >
      {value || '·'}
    </div>
...
function SafeActionButton({
  ...
  const toneMap = {
    ...
    safe: {
      bg: active ? t.greenBg : t.surface2,
      border: active ? t.green : t.border,
      color: active ? t.greenText : t.textMuted,
    },
    warning: {
      bg: active ? t.amberBg : t.surface2,
      border: active ? t.amber : t.border,
      color: active ? t.amberText : t.textMuted,
    },
    info: {
      bg: active ? t.accentBg : t.surface2,
      border: active ? t.accent : t.border,
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 8,
        background: bg,
        border: `1px solid ${tone === 'safe' ? t.green : t.amber}33`,
      }}
    >
      <div style={{ fontSize: 10, marginBottom: 3 }} className="text-t-text-subtle">
        {label}
      </div>
